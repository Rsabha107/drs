...
class RfcRequestMail extends Mailable implements ShouldQueue
{
    ...
    {
        return $this->subject('VAPP Ready for Collection ('.$this->vappRequest['event'].', ' . $this->vappRequest['request_ref_number'] .', '.$this->vappRequest['request_status'].')')
                    ->view('emails.rfc_request')
                    ->attach(
                        $this->qrFilePath,
                        [
