...
<div class="d-flex justify-content-between align-items-center m-2 mb-3">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-style1 mb-0">
            ...
            <li class="breadcrumb-item active">New Run Sheet</li>
        </ol>
    </nav>
</div>
...
<div class="card mx-2" style="max-width:700px;">
    <div class="card-header">
        <h5 class="mb-0">Create Daily Run Sheet</h5>
    </div>
    <div class="card-body">
        @if($errors->any())
        ...
        <form action="{{ route('drs.drs.store') }}" method="POST">
            @csrf
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="venue_id" class="form-select" required>
                        ...
                        @foreach($venues as $v)
                            <option value="{{ $v->id }}" {{ old('venue_id') == $v->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <select name="sheet_type" class="form-select" required>
                        @foreach(['MD-3','MD-2','MD-1','MD','MD FINAL','MD+1'] as $t)
                            <option value="{{ $t }}" {{ old('sheet_type') == $t ? 'selected' : '' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="run_date" class="form-control" value="{{ old('run_date') }}" required>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="match_id" class="form-select">
                        ...
                        @foreach($matches as $m)
                            <option value="{{ $m->id }}" {{ old('match_id') == $m->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <input type="time" name="gates_opening" class="form-control" value="{{ old('gates_opening') }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label">Kick-Off</label>
                    <input type="time" name="kick_off" class="form-control" value="{{ old('kick_off') }}">
                </div>
            </div>

            <div class="mt-4 d-flex gap-2">
                <button type="submit" class="btn btn-primary">Create & Add Items</button>
                <a href="{{ route('drs.drs.index') }}" class="btn btn-phoenix-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
